...
Shader "AmplifyShaderPack/Interior Mapping LatLong"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			sampler2D _LatLongMap0;
			sampler2D _LatLongMap1;
			sampler2D _LatLongMap2;
			sampler2D _FacadeMap;
			sampler2D _Roof;
						 ...
						 ) : SV_Target
			{
				...
				float3 worldToTangentDir182_g135 = mul( ase_worldToTangent, reflect( WorldViewDirection , normalizedWorldNormal ));
				float3 temp_output_173_0_g135 = ( float3( 1,1,1 ) / ( worldToTangentDir182_g135 * float3( -1,1,1 ) ) );
				float2 temp_output_62_0_g135 = IN.ase_texcoord7.xy;
				...
				float2 appendResult192_g135 = (float2(( atan2( break197_g135.z , break197_g135.x ) + 1.570796 ) , ( ( -0.5 * PI ) + asin( (worldToTangentDir182_g135).y ) )));
				...
				float4 tex2DNode178 = tex2D( _FacadeMap, ( ( IN.ase_texcoord7.xy * (_FacadeTilling).xy ) + (_FacadeTilling).zw ) );
				float fresnelNdotV27 = dot( WorldNormal, WorldViewDirection );
				...
				float2 uv_Roof = IN.ase_texcoord7.xy * _Roof_ST.xy + _Roof_ST.zw;
				float4 lerpResult195 = lerp( float4( lerpResult408 , 0.0 ) , tex2D( _Roof, uv_Roof ) , WorldNormal.y);
				...
				float3 lerpResult203 = lerp( lerpResult411 , float3( 0,0,0 ) , WorldNormal.y);
				
				float FacadeAlpha175 = tex2DNode178.a;
				float lerpResult412 = lerp( 0.0 , FacadeAlpha175 , _EnableFacade);
				float lerpResult204 = lerp( lerpResult412 , 0.0 , WorldNormal.y);
				

				float3 BaseColor = lerpResult195.rgb;
				float3 Normal = float3(0, 0, 1);
				float3 Emission = lerpResult203;
				float3 Specular = 0.5;
				float Metallic = lerpResult204;
				float Smoothness = lerpResult204;
				float Occlusion = 1;
				float Alpha = 1;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float3 worldToTangentDir182_g135 = mul( ase_worldToTangent, reflect( ase_worldViewDir , normalizedWorldNormal ));
				float3 temp_output_173_0_g135 = ( float3( 1,1,1 ) / ( worldToTangentDir182_g135 * float3( -1,1,1 ) ) );
				float2 temp_output_62_0_g135 = IN.ase_texcoord5.xy;
				float2 appendResult182 = (float2(_RoomCountX , _RoomCountY));
				float2 temp_output_170_0_g135 = ( temp_output_62_0_g135 * appendResult182 );
				float2 temp_output_167_0_g135 = ( ( frac( temp_output_170_0_g135 ) * float2( 2,-2 ) ) - float2( 1,-1 ) );
				float3 appendResult199_g135 = (float3(temp_output_167_0_g135 , -1.0));
				...
				float3 temp_output_31_0 = (lerpResult390).rgb;
				float3 InteriorMapping403 = temp_output_31_0;
				float4 tex2DNode178 = tex2D( _FacadeMap, ( ( IN.ase_texcoord5.xy * (_FacadeTilling).xy ) + (_FacadeTilling).zw ) );
...
WireConnection;419;0;195;0
WireConnection;419;2;203;0
WireConnection;419;3;204;0
WireConnection;419;4;204;0
ASEEND*/
//CHKSM=198872F5ACBFC128A741C4CFFB5CFBA80AFD8CE3